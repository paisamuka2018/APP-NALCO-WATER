...
const jaExiste = db.prepare('SELECT COUNT(*) AS n FROM area').get().n > 0;
if (jaExiste) {
  console.log('Banco já populado, pulando seed. Apague data.sqlite para recomeçar.');
  process.exit(0);
}

const insertArea = db.prepare('INSERT INTO area (nome) VALUES (?)');
const ccnId = insertArea.run('CCN').lastInsertRowid;
const ccsId = insertArea.run('CCS').lastInsertRowid;

const insertSistema = db.prepare(
  'INSERT INTO sistema (nome, subsistema, area_id) VALUES (?, ?, ?)'
);
const insertProduto = db.prepare(`
  INSERT INTO produto_quimico
    (nome, finalidade, sistema_id, tipo_embalagem, peso_unitario_kg, densidade, estoque_kg, estoque_minimo_kg, volume_inicial_l, volume_final_l)
  VALUES (@nome, @finalidade, @sistema_id, @tipo_embalagem, @peso_unitario_kg, @densidade, @estoque_kg, @estoque_minimo_kg, @volume_inicial_l, @volume_final_l)
`);
...
const sistemas = {
  CCN: {
    'ETA COMBUSTOL': {
      sub: 'TORRE COMBUSTOL COM e S/CONTATO',
      produtos: [
    ...
    MLC: {
      sub: 'TORRE COMBUSTOL C/CONTATO (SISTEMA DIRETO MLC)',
      produtos: [
    ...
    'SISTEMA DE LAVAGEM DE GÁS DOS ALTOS FORNOS': {
      sub: null,
      produtos: [
    ...
    ETA_LD: {
      sub: 'SIST. RESFR.ETA-LD PRIMÁRIO',
      produtos: [
    ...
    'ETAL - DOSAGEM SATÉLITE LAVADOR DE GAS': {
      sub: null,
      produtos: [
  ...
  CCS: {
    ETA_SUL: {
      sub: 'ETA TL 01/02 COM E SEM CONTATO',
      produtos: [
...
const insertMany = db.transaction(() => {
  for (const [areaNome, areaId] of [['CCN', ccnId], ['CCS', ccsId]]) {
    for (const [sistemaNome, sistemaInfo] of Object.entries(sistemas[areaNome])) {
      const sistemaId = insertSistema.run(sistemaNome, sistemaInfo.sub, areaId).lastInsertRowid;
      for (const p of sistemaInfo.produtos) {
        insertProduto.run({
          nome: p.nome,
...
insertMany();
console.log('Seed concluído: áreas, sistemas e produtos carregados a partir da planilha real.');
