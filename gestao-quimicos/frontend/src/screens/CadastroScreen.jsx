...
import EditarProdutosScreen from './EditarProdutosScreen.jsx';

const OUTRO = '__outro__';

export default function CadastroScreen() {
  const [aba, setAba] = useState('sistema'); // sistema | produto | editar
  ...
  function carregarModelos() {
    fetch('/api/produtos?modelos=1').then(r => r.json()).then(setModelos);
  }

  useEffect(() => { carregarSistemas(); carregarModelos(); }, []);
  ...
  function aplicarModelo(nome) {
    ...
    if (modelo) {
      ...
      setFinalidade(modelo.finalidade || '');
      setTipoEmbalagem(modelo.tipo_embalagem);
      setPesoUnitario(modelo.peso_unitario_kg);
      setDensidade(modelo.densidade ?? '');
    }
  }
  ...
  return (
    <div>
      <div className="tabs">
        ...
        <button className={aba === 'editar' ? 'active' : ''} onClick={() => setAba('editar')}>Editar sistemas</button>
      </div>
      ...
      {aba === 'produto' && (
        <div className="card">
          <label>Produto já cadastrado (reaproveitar dados)</label>
          <select value={modeloEscolhido} onChange={e => aplicarModelo(e.target.value)}>
            <option value="">Selecione um produto existente...</option>
            {modelos.map(m => <option key={m.nome} value={m.nome}>{m.nome}</option>)}
            <option value={OUTRO}>Não está na lista (produto novo)</option>
          </select>

          <label>Nome do produto</label>
          <input
            type="text"
            placeholder="Ex: MAGNAPRO 15.11"
            value={nomeProduto}
            disabled={modeloEscolhido !== '' && modeloEscolhido !== OUTRO}
      ...
      {aba === 'editar' && <EditarProdutosScreen />}

      {mensagem && aba !== 'editar' && <p style={{ fontSize: 13, marginBottom: 12 }}>{mensagem}</p>}

      {recentes.length > 0 && aba !== 'editar' && (
        <div className="card">
